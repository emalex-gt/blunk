...
export default function Index({ brands, filters }: { brands: Paginated<Brand>; filters: { search?: string } }) {
    ...
    const submit = (event: FormEvent) => {
        ...
        if (editing) {
            form.put(route('brands.update', editing.id), {
                preserveScroll: true,
                onSuccess: () => {
                    setEditing(null);
                    form.reset();
    ...
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold text-slate-950">Marcas</h2>}>
            <Head title="Marcas" />
            ...
            <div className="mx-auto grid max-w-7xl gap-5 px-5 py-5 lg:grid-cols-[360px_1fr] sm:px-6">
                <form onSubmit={submit} className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                            Catálogo de marcas para productos e importaciones.
                        </p>
                    </div>
                ...
                <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <form
                        onSubmit={(event) => {
                            event.preventDefault();
                            router.get(route('brands.index'), { search }, { preserveState: true });
                        }}
                        ...
                        <table className="min-w-full divide-y divide-slate-100 text-sm">
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {brands.data.map((brand) => (
                                    <tr key={brand.id}>
                                        <td className="px-3 py-3 font-semibold text-slate-950">{brand.name}</td>
                                        <td className="px-3 py-3 text-slate-600">{brand.description ?? '-'}</td>
                                        <td className="px-3 py-3 text-slate-600">{brand.products_count}</td>
                                        <td className="px-3 py-3">
                                            <span className={`rounded-full px-2 py-1 text-xs font-semibold ${brand.is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
                                                {brand.is_active ? 'Activa' : 'Inactiva'}
                                            </span>
                                        </td>
                                        <td className="space-x-2 px-3 py-3 text-right">
                                            <button onClick={() => edit(brand)} className="rounded-lg px-2 py-1 font-semibold text-indigo-600 hover:bg-indigo-50">
                                                Editar
                                            </button>
                                            {brand.is_active && (
                                                <button onClick={() => router.delete(route('brands.destroy', brand.id), { preserveScroll: true })} className="rounded-lg px-2 py-1 font-semibold text-red-600 hover:bg-red-50">
                                                    Desactivar
                                                </button>
                                            )}
                                        </td>
